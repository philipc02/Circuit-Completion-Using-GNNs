...
def trainFEGIN( dataset,dataset_name,
                ...
                tracker=None, 
                representation=None):
    # TRAINING FUNCTION FOR ALL CIRCUIT REPRESENTATIONS
    
    print(f"Training FEGIN on {representation} representation")
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for data in test_loader:
                    data = data.to(device)
                    out = model(data)
                    loss = F.cross_entropy(out, data.y.view(-1), reduction='sum')
                    test_loss += loss.item()
                    
                    preds = model(data).max(1)[1]
                    epoch_preds.extend(preds.cpu().numpy())
                    epoch_labels.extend(data.y.view(-1).cpu().numpy())
            ...
            if tracker:
                tracker.log_metrics(epoch, train_loss, test_loss, 0, f1)
            if f1 > best_f1:
                best_f1 = f1
                best_epoch = epoch
                if tracker:
    ...
    print(f'Mean F1 across iterations: {np.mean(f1_itr):.4f} ± {np.std(f1_itr):.4f}')

    f1_macro = f1_score(all_labels, all_preds, average="macro")
    final_acc = accuracy_score(all_labels, all_preds)

    return (np.mean(test_losses), np.mean(f1_itr), np.std(f1_itr), 
            {'best_f1_weighted': best_f1, 'best_acc': final_acc, 'best_f1_macro': f1_macro,
             'predictions': np.array(all_preds), 'labels': np.array(all_labels)})
