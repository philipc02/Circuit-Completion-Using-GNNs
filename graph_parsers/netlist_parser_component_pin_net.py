...
def netlist_to_netgraph(file_path, use_star_structure=True):
    ...
    for element in circuit.element_names:
        ...
        if comp_type == "X":
            # Actual number of nodes from the component
            nets = [str(net) for net in comp.nodes]
            # Generic pin names based on count
            pins = [f"{i+1}" for i in range(len(nets))]
        else:
...
if __name__ == "__main__":
    ...
    output_folder = "graphs_ltspice_examples/graphs_component_pin_net"
